...
type FacultyBlock = { teacherKey: string; title: string; entries: OfficialEntry[] }

export function OfficialPrintAllFaculty() {
  ...
  React.useEffect(() => {
    ...
    ;(async () => {
      ...
      try {
        ...
        // Group by teacher_code (unique key), preserve first-appearance order
        const map = new Map<string, { title: string; entries: OfficialEntry[] }>()
        for (const e of rawEntries as TimetableEntry[]) {
          const key = e.teacher_code ?? e.teacher_name ?? 'UNKNOWN'
          const title = e.teacher_name ?? key
          if (!map.has(key)) map.set(key, { title, entries: [] })
          map.get(key)!.entries.push(adaptTimetableEntry(e))
        }
        const built: FacultyBlock[] = []
        map.forEach(({ title, entries }, teacherKey) => built.push({ teacherKey, title, entries }))
    ...
    <div className="min-h-dvh bg-white text-slate-900">
      ...
      <div className="no-print border-b bg-white px-6 py-3 shadow-sm">
        <div className="mx-auto flex max-w-[1400px] items-center justify-between gap-3">
          <div>
            <div className="text-sm font-semibold">Official Timetable — All Faculty</div>
            <div className="mt-0.5 text-xs text-slate-500">
              {loading ? 'Loading…' : blocks.length > 0 ? `${blocks.length} teacher(s) · A4 landscape` : 'No data'}
            </div>
          </div>
      ...
      <div className="mx-auto max-w-[1400px] px-6 py-6">
        {!runId ? (
          ...
          <div className="rounded-xl border bg-slate-50 p-4 text-sm text-slate-700">Loading all faculty timetables…</div>
        ) : error ? (
