...
export function OfficialPrintAllSections() {
  ...
  const [blocks, setBlocks] = React.useState<SectionBlock[]>([])
  ...
  React.useEffect(() => {
    ...
    ;(async () => {
      ...
      try {
        ...
        const built: SectionBlock[] = []
        map.forEach(({ title, entries }, sectionCode) => built.push({ sectionCode, title, entries }))
        built.sort((a, b) => a.sectionCode.localeCompare(b.sectionCode))
    ...
    <div className="min-h-dvh bg-white text-slate-900">
      ...
      <div className="mx-auto max-w-[1400px] px-6 py-6">
        {!runId ? (
          ...
          <div className="rounded-xl border bg-slate-50 p-4 text-sm text-slate-700">Loading all section timetables…</div>
        ) : error ? (
